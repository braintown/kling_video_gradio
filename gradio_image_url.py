...
def process_image_to_video(token, model, image, tail_image, prompt, negative, mode, duration, weight,
    ...
    async def upload_images():
        img_url = await upload_image_to_blob(image, url_img) if image is not None else ""
        tail_img_url = await upload_image_to_blob(tail_image, url_img) if tail_image is not None else ""
